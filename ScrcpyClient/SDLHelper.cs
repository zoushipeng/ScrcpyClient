...
namespace ScrcpyClient
{
    public unsafe class SDLHelper
    {
        ...
        public SDLHelper(int ScreenWidth, int ScreenHeight, IntPtr handle) {
            mWidth = ScreenWidth;
        ...
        public int SDL_Init(int videoWidth, int videoHeight)
        {
            lock (this)
            {
                if (!isInit)
                ...
                {
                    screen = SDL.SDL_CreateWindow("Simplest Video Play SDL2", 0, 0, mWidth, mHeight, SDL.SDL_WindowFlags.SDL_WINDOW_OPENGL | SDL.SDL_WindowFlags.SDL_WINDOW_RESIZABLE);
                }
                SDL.SDL_ShowWindow(screen);

                SDL.SDL_SetWindowSize(screen, mWidth, mHeight);
                ...
                //创建纹理 
                sdltexture = SDL.SDL_CreateTexture(sdlrenderer, SDL.SDL_PIXELFORMAT_IYUV, (int)SDL.SDL_TextureAccess.SDL_TEXTUREACCESS_STREAMING, videoWidth, videoHeight);
                #endregion
